...
int Write_10(unsigned long	lUSB_LUN_Write)
{
	...
	else 
	{
		...
		if ((SCSI_LBA + SCSI_Transfer_Length)>ptr_USB_LUN_Write->ulNandSizeSec)	//check address overflow	
		{
			Sense_Code = 0x1B;//LOGICAL BLOCK ADDRESS OUT OF RANGE
			ret = 1;			
			goto End;
		...
		{				
			...
			ret=Receive_From_USB_DMA_USB(USB_RW_Buffer_PTR_B, USBDMA_CHANNEL1);
			if (ret != 0x00)
			{
				*P_USBD_EPEvntClear=0x0010;			
				*P_USBD_EPEvent=0x2000;		// Bulk OUT NACK Reset
			...
			*P_USBD_EPEvent=0x2000;		// Bulk OUT NACK Reset
			...
			if (!Erro_Flag)				
				ret1 = _NAND_WriteSector_USB((unsigned long)SCSI_LBA+j, 1, (unsigned long)USB_RW_Buffer_PTR_B,(unsigned int)USBDMA_CHANNEL0,stage);
